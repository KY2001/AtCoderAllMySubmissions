...
int main() {
  ...
  for (int i = 0; i < n; i++) {
    cin >> x[i] >> y[i];
    check[to_string(x[i]) + "," + to_string(y[i])] = 1;
  }
  double ans = 0;
  int x1, y1, x2, y2;
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      if (i != j) {
        ...
        if (0 <= y1 - y2 + x1 && y1 - y2 + x1 <= 5000 && 0 <= x2 - x1 + y1 &&
            x2 - x1 + y1 <= 5000) {
          if (check[to_string(y1 - y2 + x1) + "," + to_string(x2 - x1 + y1)] ==
              1) {
            int temp_x1 = x1;
            int temp_y1 = y1;
            ...
            if (0 <= y1 - y2 + x1 && y1 - y2 + x1 <= 5000 &&
              ...
              if (check[to_string(y1 - y2 + x1) + "," +
                        to_string(x2 - x1 + y1)] == 1) {
        ...
        if (0 <= x1 - y1 + y2 && x1 - y1 + y2 <= 5000 && 0 <= y1 - x2 + x1 &&
            y1 - x2 + x1 <= 5000) {
          if (check[to_string(x1 - y1 + y2) + "," + to_string(y1 - x2 + x1)] ==
              1) {
            int temp_x1 = x1;
            int temp_y1 = y1;
            x1 = -temp_y1 + y2 + temp_x1;
            ...
            if (0 <= x1 - y1 + y2 && x1 - y1 + y2 <= 5000 &&
              ...
              if (check[to_string(x1 - y1 + y2) + "," +
                        to_string(y1 - x2 + x1)] == 1) {
                ans = max((double)ans, pow(double(x1 - x2), 2.0) +
                                           pow(double(y1 - y2), 2.0));
              }
            }
