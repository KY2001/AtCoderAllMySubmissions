...
signed main() {
    ...
    V<bool> v(N + M - 1);
    fill(v.end() - (M - 1), v.end(), true);
    do {
        V<int> selected;
        rep(i, N + M - 1) if (v[i]) selected.emplace_back(i);
        selected.emplace_back(N + M - 1);
        V<int> perm;
        rep(i, selected[0]) perm.emplace_back(0);
        rep2(i, 1, selected.size()) rep(j, selected[i] - selected[i - 1] - 1) perm.emplace_back(i);
        int score = 0;
