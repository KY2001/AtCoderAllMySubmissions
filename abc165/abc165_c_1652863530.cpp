...
signed main() {
    ...
    std::fill(v.end() - (M - 1), v.end(), true);
    do {
        V<int> selected;
        for (int i = 0; i < N + M - 1; ++i)
            if (v[i]) selected.emplace_back(i);
        V<int> perm;
        rep(i, selected[0]) perm.emplace_back(1);
        rep2(i, 1, selected.size()) rep(j, selected[i] - selected[i - 1] - 1) perm.emplace_back(i + 1);
        rep2(i, selected.back() + 1, N + M - 1) perm.emplace_back(M);
        int score = 0;
        rep(i, Q) if (perm[b[i] - 1] - perm[a[i] - 1] == c[i]) score += d[i];
        chmax(ans, score);
