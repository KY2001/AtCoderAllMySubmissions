...
private:
  ...
  void calc_subtree_size_init() {
    already.resize(graph.size());
...
public:
  tree(vector<vector<vector<int>>> &graph): graph(graph) {
    Euler_Tour();             // 使わない場合削除
    dis_init();               //使わない場合削除
    calc_subtree_size_init(); //使わない場合削除
    calc_subtree_size(0);     //使わない場合削除
  }
  ...
  int calc_subtree_size(int a) { // ノードaを根としたときの各ノードの部分木の大きさを求める。
    already[a] = true;
    int ret    = 1;
    for (auto &i: graph[a]) {
      if (already[i[0]]) continue;
      ret += calc_subtree_size(i[0]);
    }
    subtree_size[a] = ret;
    return ret;
  }
...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    tree Tree(graph);
    int ret = 0;
    rep2(i, 1, N) ret += Tree.distance_from_root(i);
    auto dfs = [&](auto self, int now, int prev, int prev_score) -> void {
      cout << prev_score + N - 2 * Tree.subtree_size[now] << endl;
      rfor(i, graph[now]) {
        if (i[0] == prev) continue;
        self(self, i[0], now, prev_score + N - 2 * Tree.subtree_size[now]);
      }
    };
    dfs(dfs, 0, 0, ret + N);
  }
  ...
  return 0;
			...
			<div class="panel panel-default"><table class="table table-bordered table-striped">
				<tr>
					...
					<td class="text-center"><time class='fixtime fixtime-second'>2021-09-26 23:27:17+0900</time></td>
				</tr>
				<tr>
					<th>Task</th>
				...
				<tr>
					<th>Code Size</th>
					<td class="text-center">8546 Byte</td>
				</tr>
				<tr>
					<th>Status</th>
					...
					<tr>
						<th>Exec Time</th>
						<td class="text-center">475 ms</td>
					</tr>
					<tr>
						<th>Memory</th>
						<td class="text-center">97892 KB</td>
					</tr>
...
./Main.cpp:38:16: warning: comparison of integer expressions of different signedness: ‘long long int’ and ‘std::vector<std::vector<long long int> >::size_type’ {aka ‘long unsigned int’} [-Wsign-compare]
   38 |       while (n < v.size()) n *= 2;
      |              ~~^~~~~~~~~~
./Main.cpp:40:25: warning: comparison of integer expressions of different signedness: ‘long long int’ and ‘std::vector<std::vector<long long int> >::size_type’ {aka ‘long unsigned int’} [-Wsign-compare]
   40 |       for (int i = 0; i < v.size(); i++) data[i + n - 1] = v[i];
      |                       ~~^~~~~~~~~~
./Main.cpp: In member function ‘void tree::LCA_init()’:
./Main.cpp:109:23: warning: comparison of integer expressions of different signedness: ‘long long int’ and ‘std::vector<long long int>::size_type’ {aka ‘long unsigned int’} [-Wsign-compare]
  109 |     for (int i = 0; i < visit.size(); i++) depth_index[i] = {depth[i], visit[i]};
      |                     ~~^~~~~~~~~~~~~~
./Main.cpp: In member function ‘void tree::dis_init()’:
./Main.cpp:114:23: warning: comparison of integer expressions of different signedness: ‘long long int’ and ‘std::vector<long long int>::size_type’ {aka ‘long unsigned int’} [-Wsign-compare]
  114 |     for (int i = 0; i < e_cost2.size(); i++) bit1.add(i, e_cost2[i]);
      |                     ~~^~~~~~~~~~~~~~~~
./Main.cpp: In member function ‘void tree::subtree_init()’:
./Main.cpp:118:23: warning: comparison of integer expressions of different signedness: ‘long long int’ and ‘std::vector<long long int>::size_type’ {aka ‘long unsigned int’} [-Wsign-compare]
  118 |     for (int i = 0; i < e_cost1.size(); i++) bit2.add(i, e_cost1[i]);
      |                     ~~^~~~~~~~~~~~~~~~
