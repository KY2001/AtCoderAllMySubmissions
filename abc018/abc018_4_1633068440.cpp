...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    int N, M, P, Q, R, x, y, ans = 0;
    cin >> N >> M >> P >> Q >> R;
    VV<int> z(20, V<int>(20));
    rep(i, R) {
      cin >> x >> y;
      cin >> z[x][y];
    }
    V<int> comb;
    rep(i, N) comb.emplace_back(i + 1);
    do {
      VV<int> rank(M);
      rep(i, M) rank[i] = {0, i + 1};
      rep(i, P) rep(j, M) rank[j][0] += z[comb[i]][j+1];
      sort(all(rank), greater{});
      int val = 0;
      rep(i, Q) val += rank[i][0];
      chmax(ans, val);
    } while (next_combination(all(comb), P));
    cout << ans << endl;
  }
};
