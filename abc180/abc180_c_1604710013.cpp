...
void product(vector<int> &change, vector<unsigned int> &a, vector<unsigned int> &b, vector<vector<unsigned int>> &ab, int now = 0) {
  int a_size = a.size();
  if (now == a_size) {
    int ret = 1;
...
void enumerate_divisor(unsigned int n) {
  divisor           = {};
  auto prime_factor = FastPrimeFactorization::prime_factor(n);
  sort(ALL(prime_factor));
  vector<unsigned int> a; //a^b
  vector<unsigned int> b;
  vector<vector<unsigned int>> ab; //ab[i][j] = a[i]^j
