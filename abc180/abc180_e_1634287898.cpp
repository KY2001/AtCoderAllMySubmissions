  ...
  static void solve(istream &cin, ostream &cout) {
    ...
    rep(i, (1 << N) - 1) {
      rep(j, N) {
        ...
        rep(k, N) {
          if ((i >> k) & 1) continue;
          dp[k][i | (1 << k)] = min(dp[k][i | (1 << k)], dp[j][i] + abs(X[k] - X[j]) + abs(Y[k] - Y[j]) + max(0LL, Z[k] - Z[j]));
        }
      }
    }
    int ans = INF;
    rep(i, N) chmin(ans, dp[i][(1 << N) - 1] + abs(X[0] - X[i]) + abs(Y[0] - Y[i]) + max(0LL, Z[0] - Z[i]));
    cout << ans << endl;
  }
};
