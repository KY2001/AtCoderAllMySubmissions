...
signed main() {
    ...
    VV<int> dp((1LL << N), V<int>(N, INF));
    rep(i, N) dp[1][0] = 0;
    rep2(i, 1, (1LL << N)) {
        rep(j, N) {
            if (((i >> j) & 1) == 0) continue;
            rep(k, N) if (((i >> k) & 1) == 0) {
                chmin(dp[i xor (1LL << k)][k], dp[i][j] + abs(X[j] - X[k]) + abs(Y[j] - Y[k]) + max(0LL, Z[k] - Z[j]));
            }
        }
    }
    int ans = INF;
    rep(i, N) chmin(ans, dp[(1LL << N) - 1][i] + abs(X[0] - X[i]) + abs(Y[0] - Y[i]) + max(0LL, Z[0] - Z[i]));
    cout << ans << endl;
}