...
signed main() {
    ...
    MIN = 1e18;
    rep2(i, 2, N) if (dis2[i] == 1) chmin(MIN, dis[i]);
    cnt = 0;
    rep2(i, 2, N) if (dis2[i] == 1 and MIN == dis[i]) cnt++;
    if (cnt > 1 or _min(dis) == 2 or _min(dis2) == 2) {
        cout << "! " << 1 << endl;
        return 0;
    }
    rep2(i, 2, N) {
        if (dis2[i] == 1 and dis[i] == MIN) {
            if (1 + dis[i] != ans) {
                while (1) {
                    int k = 2;
                }
    ...
    cout << "! " << ans << endl;
			...
			<div class="panel panel-default"><table class="table table-bordered table-striped">
				<tr>
					...
					<td class="text-center"><time class='fixtime fixtime-second'>2022-06-19 22:54:49+0900</time></td>
				</tr>
				<tr>
					<th>Task</th>
				...
				<tr>
					<th>Code Size</th>
					<td class="text-center">4838 Byte</td>
				</tr>
				<tr>
					<th>Status</th>
					...
					<tr>
						<th>Memory</th>
						<td class="text-center">3848 KB</td>
					</tr>
...
./Main.cpp:89:25: warning: unused variable ‘k’ [-Wunused-variable]
   89 |                     int k = 2;
      |                         ^
./Main.cpp:98:34: warning: ‘ans_idx’ may be used uninitialized in this function [-Wmaybe-uninitialized]
   98 |         if (dis[i] == dis[ans_idx] + d) {
      |                                  ^
