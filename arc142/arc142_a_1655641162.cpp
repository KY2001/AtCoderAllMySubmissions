...
signed main() {
    ...
    string K_rev_str = K_str;
    reverse(all(K_str));
    if (stoll(K_rev_str) > stoll(K_str) or K_str.back() == '0') {
        cout << 0 << endl;
        return 0;
    }
    while (stoll(K_str) <= N) {
    ...
    while (K_str.back() == '0') K_str.pop_back();
    if ((stoll(K_rev_str) != stoll(K_str))) {
        while (stoll(K_rev_str) <= N) {
            ans++;
            K_rev_str += '0';
        }
    }
