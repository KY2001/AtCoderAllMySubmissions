...
signed main() {
  ...
  deque<V<int>> que;
  int prev = 0;
  V<int> for_check(100000 + 2);
  rep(i, N) {
    if (wh[i][0] > prev) {
      while (que.size()) {
        for_check[que[0][0]] = que[0][1];
        seg.update(que[0][0], que[0][1]);
        que.pop_front();
      }
    }
    int val = wh[i][1] == MIN ? 1 : seg.get_max(0, where[wh[i][1] - 1]) + 1;
    chmax(ans, val);
    que.push_back({where[wh[i][1] - 1] + now_h_num[wh[i][1]]++, val});
    prev = wh[i][0];
  }
  cout << ans << endl;
}</pre>
			...
			<div class="panel panel-default"><table class="table table-bordered table-striped">
				<tr>
					...
					<td class="text-center"><time class='fixtime fixtime-second'>2021-09-10 14:51:26+0900</time></td>
				</tr>
				<tr>
					<th>Task</th>
					<td class="text-center"><a href="/contests/abc038/tasks/abc038_d">D - プレゼント</a></td>
				</tr>
				<tr>
					<th>User</th>
					<td class="text-center"><a href="/users/KY2001">KY2001</a> <a href='/contests/abc038/submissions?f.User=KY2001'><span class='glyphicon glyphicon-search black' aria-hidden='true' data-toggle='tooltip' title='view ky's submissions'></span></a></td>
				</tr>
				<tr>
					<th>Language</th>
				...
				<tr>
					<th>Code Size</th>
					<td class="text-center">4632 Byte</td>
				</tr>
				<tr>
					<th>Status</th>
					...
					<tr>
						<th>Exec Time</th>
						<td class="text-center">76 ms</td>
					</tr>
					<tr>
						<th>Memory</th>
						<td class="text-center">13744 KB</td>
					</tr>
...
./Main.cpp:86:28:   required from here
