...
VV<int> graph;
V<bool> already;
VV<mint> memo(100001, V<mint>(2));

mint dfs(int now, int c) {
  if (memo[now][c].x) return memo[now][c];
  already[now] = true;
  mint ret     = 1;
  rep4(i, graph[now]) {
    if (already[i]) continue;
    ret *= dfs(i, 0) + (!c ? dfs(i, 1) : 0);
  }
  already[now] = false;
  memo[now][c] = (graph[now].size() == 1 and already[graph[now][0]]) ? 1 : ret;
  return memo[now][c];
}
