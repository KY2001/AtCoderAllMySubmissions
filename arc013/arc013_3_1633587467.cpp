...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    rep(i, N) {
      ...
      rfor(j, XYZ) cin >> j;
      cin >> M;
      VV<int> xyz(3, V<int>(M));
      rep(j, M) cin >> xyz[0][j] >> xyz[1][j] >> xyz[2][j];
      rep(j, 3) {
        if (min(_min(xyz[j]), XYZ[j] - _max(xyz[j]) - 1) != 0) {
          XOR ^= min(_min(xyz[j]) - 1, XYZ[j] - _max(xyz[j]) - 2);
        } else {
          XOR ^= max(_min(xyz[j]) + XYZ[j] - _max(xyz[j]) - 1, 1LL);
        }
      }
    }
    if (XOR) {
