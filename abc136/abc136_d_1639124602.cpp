...
class DGatheringChildren {
  public:
    static void solve(istream &cin, ostream &cout) {
        ...
        string S;
        cin >> S;
        V<P<int, int>> to(S.size(), {-1, -1});
        rep(i, S.size() - 1) if (S[i] == 'R' and S[i + 1] == 'L') to[i] = {i, i + 1}, to[i + 1] = {i + 1, i};
        rep(i, S.size()) {
            if (to[i].fi == -1) {
                int cnt = 0;
                int now = i;
                ...
                while (1) {
                    ...
                    if (to[now].first != -1) {
                        ...
                        while (came.size()) {
                            cnt++;
                            if (cnt % 2) {
                                to[came.back()] = {to[now].se, to[now].fi};
                            } else {
                                to[came.back()] = {to[now].fi, to[now].se};
                            }
                            came.pop_back();
                        }
                        break;
