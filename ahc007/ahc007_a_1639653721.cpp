...
double gap = 0.012761891419392147;
int kruskal(int now) {
...
bool judge(int u1, int v1, int u2, int v2) {
    return (uf.same(u1, u2) and uf.same(v1, v2)) or (uf.same(u1, v2) and uf.same(v1, u2));
}
void calc1() {
    ...
    rep(i, M) {
        ...
        V<int> equiv;
        rep2(j, i + 1, M) if (judge(u[i], v[i], u[j], v[j])) equiv.emplace_back(cost[j]);
        rep2(j, i + 1, M - 1) rep2(k, j + 1, M) if (v[j] == u[k] and judge(u[i], v[i], u[j], v[k])) equiv.emplace_back(cost[j] + cost[k]);
        if (equiv.size()) {
            rep(j, cost[i]) {
                double pro = 1;
                rfor(k, equiv) {
                    if (k >= j) {
                        ...
                        pro *= (1 - (double)(j - k) / (2 * k));
                    }
                }
                pro = 1 - pro;
                if (pro >= 0.5733590254310986) {
                    yes = true;
                    break;
                }
            }
