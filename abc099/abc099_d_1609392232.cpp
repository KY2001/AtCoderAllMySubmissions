...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    VV<int> change(C, V<int>(3));
    rep(i, C) rep(j, 3) {
      rep(k, N) rep(l, N) if ((k + l + 2) % 3 == j) change[i][j] += D[c[k][l]][i];
    }
    V<int> perm(C);
    rep(i, C) perm[i] = i;
    int ans           = INF;
    V<int> temp(3);
    do {
      rep(i, 3) temp[i] = perm[i];
      do {
        int ret = 0;
        rep(i, 3) ret += change[temp[i]][i];
        chmin(ans, ret);
      } while (next_permutation(ALL(temp)));
    } while (next_combination(ALL(perm), 3));
    cout << ans << endl;
  }
};
