...
void dfs(int y, int x, int now, string &road, V<bool> &already, VV<int> &t, VV<int> &p) {
  ...
  if ((clock() - start) / CLOCKS_PER_SEC > 1.95) return;
  deque<P<int, char>> temp;
  temp.emplace_back(p[y + 1][x], 'D');
  temp.emplace_back(p[y - 1][x], 'U');
  temp.emplace_back(p[y][x + 1], 'R');
  temp.emplace_back(p[y][x - 1], 'L');
