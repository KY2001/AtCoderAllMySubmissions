...
class DMazeMaster {
  public:
    static void solve(istream &cin, ostream &cout) {
        ...
        grid[0]     = string('#', W + 2);
        grid[H + 1] = string('#', W + 2);
        rep(i, H) cin >> grid[i + 1];
        rep(i, H) grid[i + 1] = '#' + grid[i + 1] + '#';
        int ans               = 0;
        rep(s, H * W) rep(g, H * W) {
            if (grid[s / W + 1][s % W + 1] == '#') continue;
            if (grid[g / W + 1][g % W + 1] == '#') continue;
            deque<P<int, int>> pos = {{s / W + 1, s % W + 1}};
            VV<int> shortest_dis(H + 2, V<int>(W + 2, INF));
            shortest_dis[s / W + 1][s % W + 1] = 0;
            while (pos.size()) {
                int x = pos[0].fi, y = pos[0].se;
                pos.pop_front();
                if (grid[x + 1][y] == '.' and shortest_dis[x + 1][y] > shortest_dis[x][y] + 1) {
                    shortest_dis[x + 1][y] = shortest_dis[x][y] + 1;
                    pos.emplace_back(x + 1, y);
                }
                if (grid[x - 1][y] == '.' and shortest_dis[x - 1][y] > shortest_dis[x][y] + 1) {
                    shortest_dis[x - 1][y] = shortest_dis[x][y] + 1;
                    pos.emplace_back(x - 1, y);
                }
                if (grid[x][y + 1] == '.' and shortest_dis[x][y + 1] > shortest_dis[x][y] + 1) {
                    shortest_dis[x][y + 1] = shortest_dis[x][y] + 1;
                    pos.emplace_back(x, y + 1);
                }
                if (grid[x][y - 1] == '.' and shortest_dis[x][y - 1] > shortest_dis[x][y] + 1) {
                    shortest_dis[x][y - 1] = shortest_dis[x][y] + 1;
                    pos.emplace_back(x, y - 1);
            ...
            chmax(ans, shortest_dis[g / W + 1][g % W + 1]);
        }
        cout << ans << endl;
    }
};
