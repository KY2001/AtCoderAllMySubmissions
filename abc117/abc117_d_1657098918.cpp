...
signed main() {
    ...
    int digit   = 50;
    int one_max = 0;
    rep3(i, digit, 0) if ((K >> i) & 1) {
        one_max = i;
        break;
    }
    V<int> zeros(digit);
    rep(i, digit) rfor(j, A) if (((j >> i) & 1) == 0) zeros[i]++;
    int ans_up = 0;
    rep3(i, digit - 1, one_max + 1) {
        ans_up += (N - zeros[i]) * (1LL << i);
    }
    int ans = 0;
    rep3(i, one_max, 0) {
        if (((K >> i) & 1) == 0) continue;
        int temp = 0;
        rep3(j, one_max, i + 1) temp += (((K >> j) & 1) ? zeros[j] : N - zeros[j]) * (1LL << j);
        temp += (1LL << i) * (N - zeros[i]);
        rep3(j, i - 1, 0) temp += max(zeros[j], N - zeros[j]) * (1LL << j);
        chmax(ans, temp);
    }
    int temp = 0;
    rep3(i, one_max, 0) temp += (((K >> i) & 1) ? zeros[i] : N - zeros[i]) * (1LL << i);
    chmax(ans, temp);
    cout << ans + ans_up << endl;
}