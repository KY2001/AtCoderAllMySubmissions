...
signed main() {
    ...
    rfor(i, X) i--;
    V<int> value(N);
    rep(i, N) value[X[i]] += C[i];
    set<P<int, int>> list;
    rep(i, N) list.insert({value[i], i});
    int ans = 0;
    while (list.size()) {
        P<int, int> now = *begin(list);
        list.erase(now);
        ans += now.fi;
        value[now.se] = -1;
        if (value[X[now.se]] != -1) {
            list.erase({value[X[now.se]], X[now.se]});
            list.insert({value[X[now.se]] - C[now.se], X[now.se]});
            value[X[now.se]] -= C[now.se];
        }
    }
    cout << ans << endl;
}