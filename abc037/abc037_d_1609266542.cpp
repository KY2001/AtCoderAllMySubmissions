...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    int H, W;
    cin >> H >> W;
    VV<int> a(H + 2, V<int>(W + 2, 0));
    VV<bool> already(H + 2, V<bool>(W + 2));
    VV<mint> ans(H + 2, V<mint>(W + 2, 0));
    ...
    while (pos.size()) {
      ...
      pos.pop_front();
      if (already[y][x]) num -= 1;
      already[y][x] = true;
      ans[y][x] += num;
      if (a[y][x] > a[y + 1][x] and a[y + 1][x] != 0) pos.push_back({y + 1, x, num + 1});
      if (a[y][x] > a[y - 1][x] and a[y - 1][x] != 0) pos.push_back({y - 1, x, num + 1});
      if (a[y][x] > a[y][x + 1] and a[y][x + 1] != 0) pos.push_back({y, x + 1, num + 1});
      if (a[y][x] > a[y][x - 1] and a[y][x - 1] != 0) pos.push_back({y, x - 1, num + 1});
    }
    mint ret = 0;
    rep(i, H) rep(j, W) ret += ans[i + 1][j + 1];
    cout << ret << endl;
  }
};
