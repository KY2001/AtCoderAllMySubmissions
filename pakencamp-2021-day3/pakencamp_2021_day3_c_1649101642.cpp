...
int func(int R, int M) {
    ...
    rep(i, M) {
        set<int> cnt;
        V<int> cnt2;
        rep(j, pow_of_five[i].size()) {
            if (cnt.count(pow_of_five[i][j])) {
                ans += ((R - start[i]) / j) * _sum(cnt2);
                rep(k, (R - start[i]) % j) ans += cnt2[k];
                break;
            }
            cnt.insert(pow_of_five[i][j]);
            cnt2.emplace_back(pow_of_five[i][j]);
        }
    }
    return ans;
}
    ...
    cout << func(R, M) - func(L - 1, M) << endl;
			...
			<div class="panel panel-default"><table class="table table-bordered table-striped">
				<tr>
					...
					<td class="text-center"><time class='fixtime fixtime-second'>2022-04-05 04:47:22+0900</time></td>
				</tr>
				<tr>
					<th>Task</th>
				...
				<tr>
					<th>Code Size</th>
					<td class="text-center">4245 Byte</td>
				</tr>
				<tr>
					<th>Status</th>
					...
					<tr>
						<th>Memory</th>
						<td class="text-center">3888 KB</td>
					</tr>
