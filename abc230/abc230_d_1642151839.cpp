...
class DDestroyerTakahashi {
  public:
    static void solve(istream &cin, ostream &cout) {
        ...
        V<int> L(N), R(N);
        rep(i, N) cin >> L[i] >> R[i];
        V<P<int, int>> R_index(N);
        deque<P<int, int>> L_index(N);
        rep(i, N) R_index[i] = {R[i], i};
        rep(i, N) L_index[i] = {L[i], i};
        sort(all(R_index));
        sort(all(L_index));
        V<bool> valid(N, true);
        int ans = 0;
        rfor(i, R_index) {
            if (not valid[i.se]) continue;
            ans++;
            if (L_index.size()) {
                while (L_index[0].fi <= i.fi + D - 1) {
                    valid[L_index[0].se] = false;
                    L_index.pop_front();
                    if (L_index.empty()) break;
                }
            }
        }
        cout << ans << endl;
