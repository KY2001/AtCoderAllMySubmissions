...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    vector<int> A(N);
    rep(i, N) cin >> A[i];
    vector<int> cum_gcd(N);
    vector<int> ans(N);
    cum_gcd[0]                   = A[0];
    rep(i, N - 1) cum_gcd[i + 1] = gcd(cum_gcd[i], A[i + 1]);
    ans[N - 1]                   = cum_gcd[N - 2];
    int cum_gcd_reverse          = A[N - 1];
    rep(i, N - 2) {
      ans[N - 2 - i]  = gcd(cum_gcd[N - 3 - i], cum_gcd_reverse);
      cum_gcd_reverse = gcd(cum_gcd_reverse, A[N - 2 - i]);
    }
    ans[0] = cum_gcd_reverse;
    cout << MAX(ans) << endl;
  }
};
