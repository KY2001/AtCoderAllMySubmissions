...
signed main() {
    ...
    double A, B, C;
    cin >> A >> B >> C;
    int L_init = 0, R_init = INF; // [L_init, R_init]で広義単調増加
    int L = L_init - 1, R = R_init + 1;
    while (R - L > 1) {
        int mid   = (L + R) / 2;
        double t  = 1 / (PI * C) * (mid * PI + acos(-A / (PI * B * C)));
    ...
    // Rが答えになる
    double t  = 1 / (PI * C) * (R * PI + acos(-A / (PI * B * C)));
    double ft = A * t + B * sin(C * t * PI);
    if (ft >= 100 - EPS) {
        double L2 = 1 / (PI * C) * ((R - 1) * PI + acos(-A / (PI * B * C))), R2 = t;
        while (R2 - L2 > EPS) {
            double mid = (L2 + R2) / 2;
