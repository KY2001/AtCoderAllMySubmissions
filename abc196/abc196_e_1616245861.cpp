  ...
  static void solve(istream &cin, ostream &cout) {
    ...
    rep3(i, N - 1, 0) {
      while (1) {
        ...
        int maxi = Upper_bound(max, max[i]);
        int mini = Upper_bound_reverse(min, max[i]);
        ...
        if (maxi == INF and mini == INF) {
          ans[MAX[i]] = max[i] + a_sum[N];
          ans[max[i]] = max[i] + a_sum[N];
          break;
        } else if (maxi < mini) {
          max[i] = a[maxi] - a_sum[maxi];
        } else {
          max[i] = a[mini] - a_sum[mini];
        }
      }
    }
    rep3(i, N - 1, 0) {
      while (1) {
        ...
        int maxi = Upper_bound(max, min[i]);
        int mini = Upper_bound_reverse(min, min[i]);
        if (maxi == -1 or max[maxi] == INF) maxi = INF;
        if (mini == -1 or min[mini] == INF) {
          mini = INF;
        } else {
        ...
        if (maxi == INF and mini == INF) {
          ans[MIN[i]] = min[i] + a_sum[N];
          ans[min[i]] = min[i] + a_sum[N];
          break;
        } else if (maxi < mini) {
          min[i] = a[maxi] - a_sum[maxi];
        } else {
          min[i] = a[mini] - a_sum[mini];
        }
      }
    }
    rep(i, Q) {
  ...
  return 0;
			...
			<div class="panel panel-default"><table class="table table-bordered table-striped">
				<tr>
					...
					<td class="text-center"><time class='fixtime fixtime-second'>2021-03-20 22:11:01+0900</time></td>
				</tr>
				<tr>
					<th>Task</th>
				...
				<tr>
					<th>Code Size</th>
					<td class="text-center">7043 Byte</td>
				</tr>
				<tr>
					<th>Status</th>
					...
					<tr>
						<th>Memory</th>
						<td class="text-center">30480 KB</td>
					</tr>
...
./Main.cpp:107:43:   required from here
...
./Main.cpp:108:51:   required from here
