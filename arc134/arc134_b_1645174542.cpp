...
signed main() {
    ...
    string S;
    cin >> N >> S;
    V<set<int>> alphabet_indexes(26);
    rep(i, N) alphabet_indexes[S[i] - 'a'].insert(i);
    int R      = N;
    string ans = S;
    rep(L, N) {
        if (L == R) break;
        rep(i, S[L] - 'a') {
            if (alphabet_indexes[i].empty()) continue;
            auto r = prev(alphabet_indexes[i].lower_bound(R));
            if (*r >= R or *r <= L) continue;
            swap(ans[L], ans[*r]);
            R = *r;
            alphabet_indexes[i].erase(r);
            break;
        }
    }
    cout << ans << endl;
