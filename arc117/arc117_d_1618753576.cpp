...
int dis_max;
int max_node;
void dfs1(int now, int now_dis, VV<int> &graph, V<bool> &already) {
  already[now] = true;
  now_dis++;
  if (now_dis > dis_max) {
    dis_max  = now_dis;
  ...
  FOR(i, graph[now]) {
    if (!already[i]) dfs1(i, now_dis, graph, already);
  }
}

int dfs2(int now, int now_dis, int max_dis, int a, VV<int> &graph, V<bool> &already, V<int> &ans) {
  already[now] = true;
  now_dis++;
  a++;
  ans[now] = a;
  if (a == now_dis) chmax(max_dis, now_dis);
  FOR(i, graph[now]) {
    if (!already[i]) {
      chmax(max_dis, dfs2(i, now_dis, max_dis, now_dis + (max_dis - now_dis), graph, already, ans));
    }
  }
  return max_dis;
}
...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    dis_max  = -1;
    max_node = 0;
    dfs1(0, -1, graph, already);
    int R                = max_node;
    rep(i, N) already[i] = false;
    dis_max              = -1;
    dfs1(R, -1, graph, already);
    int L = max_node;
    V<int> ans(N);
    dis_max              = -1;
    rep(i, N) already[i] = false;
    dfs2(L, 0, 0, 0, graph, already, ans);
    FOR(i, ans)
    cout << i << " ";
    cout << endl;
  }
  ...
  return 0;
			...
			<div class="panel panel-default"><table class="table table-bordered table-striped">
				<tr>
					...
					<td class="text-center"><time class='fixtime fixtime-second'>2021-04-18 22:46:16+0900</time></td>
				</tr>
				<tr>
					<th>Task</th>
					<td class="text-center"><a href="/contests/arc117/tasks/arc117_d">D - Miracle Tree</a></td>
				</tr>
				<tr>
					<th>User</th>
					<td class="text-center"><a href="/users/KY2001">KY2001</a> <a href='/contests/arc117/submissions?f.User=KY2001'><span class='glyphicon glyphicon-search black' aria-hidden='true' data-toggle='tooltip' title='view KY2001's submissions'></span></a></td>
				</tr>
				<tr>
					<th>Language</th>
				...
				<tr>
					<th>Code Size</th>
					<td class="text-center">4271 Byte</td>
				</tr>
				<tr>
					<th>Status</th>
