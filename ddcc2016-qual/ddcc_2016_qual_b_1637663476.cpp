...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    cin >> R >> N >> M;
    rep(i, N + M - 1) {
      if (1 <= i + 1 and i + 1 < N - 1 and 1 <= i + 1 - M and i + 1 - M <= N - 1) {
        ans += max(2 * sqrt(pow(R, 2) - pow(R, 2) * pow((1.0 - 2 * (double)(i + 1) / N), 2)), 2 * sqrt(pow(R, 2) - pow(R, 2) * pow((1.0 - 2 * (double)(i + 1 - M) / N), 2)));
      } else if (1 <= i + 1 and i + 1 < N - 1) {
        ans += 2 * sqrt(pow(R, 2) - pow(R, 2) * pow((1.0 - 2 * (double)(i + 1) / N), 2));
      } else {
        ans += 2 * sqrt(pow(R, 2) - pow(R, 2) * pow((1.0 - 2 * (double)(i + 1 - M) / N), 2));
      }
    }
    cout << ans << endl;
  }
