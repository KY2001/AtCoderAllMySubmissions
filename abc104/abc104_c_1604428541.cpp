...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    rep(i, 1 << D) {
      int score = 0;
      int cost  = 0;
      vector<pair<int, int>> not_use_p{};
      rep(j, D) {
        if ((i >> j) & 1) {
          score += c[j] + 100 * (j + 1) * p[j];
          cost += p[j];
        } else {
          not_use_p.emplace_back(100 * (j + 1), p[j]);
        }
      }
      if (score >= G) {
        ans = min(ans, cost);
        ...
        sort(ALL(not_use_p));
        if (not_use_p[not_use_p.size() - 1].first * not_use_p[not_use_p.size() - 1].second >= G - score) {
          ans = min(ans, cost + CEIL(G - score, not_use_p[not_use_p.size() - 1].first));
        }
      }
    }
    cout << ans << endl;
