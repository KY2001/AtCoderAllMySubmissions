...
using namespace std;
struct _edge {
  int long long to, cost;
};
typedef pair<int long long, int long long> _P;
int long long _V;       //do assign num of nodes
#define MAX_V 100000    //do change MAX_V if necessary
vector<_edge> G[MAX_V]; // do G[from] = {to, cost}
int long long shortest_path[MAX_V];
void dijkstra(int long long s) {
...
int long long check[100000]{};
int main() {
  int long long N, M, K, S, P, Q;
  cin >> N >> M >> K >> S >> P >> Q;
  _V = N;
  vector<int long long> C(K);
  ...
  deque<_P> pos;
  int long long check2[100000]{};
  for (int long long i = 0; i < K; i++) {
    pos.emplace_back(C[i] - 1, 0);
    check2[C[i] - 1] = 1;
  }
  int long long now, dis;
  while (!pos.empty()) {
    now = pos[0].first;
    dis = pos[0].second;
    pos.pop_front();
