...
void dfs(int now, int now_xor, VV<P<int, int>> &graph, V<bool> &already, map<int, int> &check) {
  already[now] = true;
  check[now_xor]++;
  FOR(i, graph[now]) {
    if (!already[i.fi]) {
      dfs(i.fi, now_xor ^ i.se, graph, already, check);
    }
  }
}
...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    V<bool> already(N);
    map<int, int> check;
    dfs(0, 0, graph, already, check);
    int ans = 0;
    FOR(i, check) {
      if (i.fi == (X ^ i.fi)) {
        ans += i.se * (i.se - 1) / 2;
      } else {
        ans += check[X ^ i.fi] * i.se;
      }
    }
    cout << ans / 2 << endl;
  }
