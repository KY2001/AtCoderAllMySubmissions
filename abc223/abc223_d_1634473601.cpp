...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    rep(i, M) {
      cin >> A[i] >> B[i];
      graph1[A[i] - 1].emplace_back(B[i] - 1);
      graph2[B[i] - 1].emplace_back(A[i] - 1);
    }
    rep(i, N) {
      sort(all(graph1[i]));
      sort(all(graph2[i]));
    ...
    auto dfs = [&](auto self, int now) -> void {
      if (!check[now]) {
        bool yes = true;
        rfor(j, graph2[now]) {
          if (!check[j]) yes = false;
        }
        if (yes) {
          ans.emplace_back(now);
          check[now] = true;
          rfor(j, graph1[now]) self(self, j);
        }
      }
    };
    rep(i, N) {
  ...
  return 0;
			...
			<div class="panel panel-default"><table class="table table-bordered table-striped">
				<tr>
					...
					<td class="text-center"><time class='fixtime fixtime-second'>2021-10-17 21:26:41+0900</time></td>
				</tr>
				<tr>
					<th>Task</th>
					<td class="text-center"><a href="/contests/abc223/tasks/abc223_d">D - Restricted Permutation</a></td>
				</tr>
				<tr>
					<th>User</th>
					<td class="text-center"><a href="/users/KY2001">KY2001</a> <a href='/contests/abc223/submissions?f.User=KY2001'><span class='glyphicon glyphicon-search black' aria-hidden='true' data-toggle='tooltip' title='view KY2001's submissions'></span></a></td>
				</tr>
				<tr>
					<th>Language</th>
				...
				<tr>
					<th>Code Size</th>
					<td class="text-center">4095 Byte</td>
				</tr>
				<tr>
					<th>Status</th>
					...
					<tr>
						<th>Exec Time</th>
						<td class="text-center">159 ms</td>
					</tr>
					<tr>
						<th>Memory</th>
						<td class="text-center">26168 KB</td>
					</tr>
				...
				<h4>Compile Error</h4>
				<pre>./Main.cpp: In static member function ‘static void DRestrictedPermutation::solve(std::istream&, std::ostream&)’:
./Main.cpp:72:20: warning: comparison of integer expressions of different signedness: ‘std::vector<long long int>::size_type’ {aka ‘long unsigned int’} and ‘long long int’ [-Wsign-compare]
   72 |     if (ans.size() != N) {
      |         ~~~~~~~~~~~^~~~
