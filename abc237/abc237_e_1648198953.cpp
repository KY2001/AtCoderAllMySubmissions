...
signed main() {
    ...
    while (pos.size()) {
        int now = pos.top().se;
        int dis = pos.top().fi;
        if (shortest_dis[now] != dis) continue;
        pos.pop();
        rfor(i, graph[now]) {
            int cost;
            if (H[i] > H[now]) cost = -2 * (H[i] - H[now]);
            if (H[i] <= H[now]) cost = H[now] - H[i];
            if (shortest_dis[i] < dis + cost) {
                shortest_dis[i] = dis + cost;
                pos.push({dis + cost, i});
                chmax(ans, dis + cost);
            }
        }
