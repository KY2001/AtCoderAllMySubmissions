...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    rep(i, M) cin >> P[i] >> Y[i];
    map<int, V<int>> check;
    rep(i, M) check[P[i]].emplace_back(Y[i]);
    rfor(i, check) sort(all(i.se));
    rep(i, M) {
      string ans = string(6 - to_string(P[i]).size(), '0') + to_string(P[i]);
      int right  = Lower_bound(check[P[i]], Y[i]) + 1;
      ans += string(6 - to_string(right).size(), '0') + to_string(right);
      cout << ans << endl;
    }
  }
};
  ...
  return 0;
			...
			<div class="panel panel-default"><table class="table table-bordered table-striped">
				<tr>
					...
					<td class="text-center"><time class='fixtime fixtime-second'>2021-10-08 17:54:45+0900</time></td>
				</tr>
				<tr>
					<th>Task</th>
					<td class="text-center"><a href="/contests/abc113/tasks/abc113_c">C - ID</a></td>
				</tr>
				<tr>
					<th>User</th>
					<td class="text-center"><a href="/users/KY2001">KY2001</a> <a href='/contests/abc113/submissions?f.User=KY2001'><span class='glyphicon glyphicon-search black' aria-hidden='true' data-toggle='tooltip' title='view KY2001's submissions'></span></a></td>
				</tr>
				<tr>
					<th>Language</th>
				...
				<tr>
					<th>Code Size</th>
					<td class="text-center">3864 Byte</td>
				</tr>
				<tr>
					<th>Status</th>
					...
					<tr>
						<th>Exec Time</th>
						<td class="text-center">142 ms</td>
					</tr>
					<tr>
						<th>Memory</th>
						<td class="text-center">15508 KB</td>
					</tr>
...
./Main.cpp:54:49:   required from here
./Main.cpp:35:28: warning: comparison of integer expressions of different signedness: ‘long long int’ and ‘std::vector<long long int>::size_type’ {aka ‘long unsigned int’} [-Wsign-compare]
   35 |   return (0 <= ind and ind < A.size()) ? ind : -1;
