...
signed main() {
  ...
  while (1) {
    k++;
    int best = INF;
    rep(i, n - 2) rep2(j, i + 1, n - 1) {
      swap(p[i], p[j]);
      chmin(best, calc_score(p, distance));
      swap(p[i], p[j]);
    }
    rep(i, n - 2) rep2(j, i + 1, n - 1) {
      swap(p[i], p[j]);
      if (calc_score(p, distance) == best) {
        if (best < score) {
          score = best;
          ret   = get_ans(p, path);
          prev = k;
  ...
  //cout << score << endl;
			...
			<div class="panel panel-default"><table class="table table-bordered table-striped">
				<tr>
					...
					<td class="text-center"><time class='fixtime fixtime-second'>2021-08-07 19:25:24+0900</time></td>
				</tr>
				<tr>
					<th>Task</th>
				...
				<tr>
					<th>Score</th>
					<td class="text-center">14905476</td>
				</tr>
				<tr>
					<th>Code Size</th>
					<td class="text-center">7517 Byte</td>
				</tr>
				<tr>
					<th>Status</th>
					...
					<tr>
						<th>Exec Time</th>
						<td class="text-center">2993 ms</td>
					</tr>
					<tr>
						<th>Memory</th>
						<td class="text-center">4460 KB</td>
					</tr>
