...
bool possible(vector<pair<double, double>> &xy, double r) {
  ...
  rep(i, N - 1) {
    rep2(j, i + 1, N) {
      if ((xy[i].first - xy[j].first) * (xy[i].first - xy[j].first) + (xy[i].second - xy[j].second) * (xy[i].second - xy[j].second) < r * r) {
        uf.unite(i, j);
        break;
      }
      rep(k, xy.size()) {
  ...
  rep(i, N) {
    if (100 - xy[i].second < r) {
      rep2(j, N, N + 201) {
        uf.unite(i, j);
      }
    }
    if (xy[i].second + 100 < r) {
...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    for (double i = -100; i < 101 - EPS; i += 0.1) {
    ...
    while (true) {
      ...
      if (one and !two or R == 0 or L == 200) {
        cout << mid / 2 << endl;
        return;
      } else if (!one and !two) {
        R = mid;
  ...
  return 0;
			...
			<div class="panel panel-default"><table class="table table-bordered table-striped">
				<tr>
					...
					<td class="text-center"><time class='fixtime fixtime-second'>2020-11-01 23:27:47+0900</time></td>
				</tr>
				<tr>
					<th>Task</th>
				...
				<tr>
					<th>Code Size</th>
					<td class="text-center">5221 Byte</td>
				</tr>
				<tr>
					<th>Status</th>
					...
					<tr>
						<th>Exec Time</th>
						<td class="text-center">476 ms</td>
					</tr>
					<tr>
						<th>Memory</th>
						<td class="text-center">3896 KB</td>
					</tr>
...
./Main.cpp:149:15: warning: suggest parentheses around ‘&&’ within ‘||’ [-Wparentheses]
  149 |       if (one and !two or R == 0 or L == 200) {
      |           ~~~~^~~~~~~~
