...
signed main() {
    ...
    V<int> a(N);
    rfor(i, a) cin >> i;
    V<P<int, int>> continuous = {{a[0], 1}};
    cout << 1 << endl;
    rep2(i, 1, N) {
        if (a[i] == continuous.back().fi) {
            continuous.push_back({a[i], continuous.back().se + 1});
        } else {
            continuous.push_back({a[i], 1});
        }
        if (continuous.back().se == a[i]) {
            rep(j, a[i]) continuous.pop_back();
        }
