  ...
  rep(i, n) {
    ...
    rep3(length, sqrt(r[now]), 1) {
      ...
      rep(j, n) {
        if (now == j)
          continue;
        if (out_of_border(ans[now][2], ans[now][3]) or x[now] <= 20 or
            y[now] <= 20) {
          possible = false;
          break;
        }
    ...
    if (!possible) {
      ans[now] = {i, i % 10, i + 1, i % 10 + 1};
    }
  }
  FOR(i, ans) {
    cout << i[0] << " " << i[1] << " " << i[2] << " " << i[3] << endl;
  }
  cout << duplicate(ans[7][0], ans[7][1], ans[7][2], ans[7][3], ans[15][0],
                    ans[15][1], ans[15][2], ans[15][3])
       << endl;
}