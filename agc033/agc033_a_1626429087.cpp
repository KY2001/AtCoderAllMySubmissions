...
void check(int now, int x, int y, deque<V<int>> &pos, VV<int> &a) {
  if (a[x][y] > now + 1 and a[x][y] != -1) {
    a[x][y] = now + 1;
    pos.push_back({now + 1, x, y});
  }
}
...
signed main() {
  ...
  deque<V<int>> pos;
  rep(i, H) rep(j, W) {
    if (a[i + 1][j + 1] == 0) pos.push_back({a[i + 1][j + 1], i + 1, j + 1});
  }
  while (pos.size()) {
    V<int> now = pos[0];
    pos.pop_front();
  ...
  int ans = 0;
  rep(i, H) rep(j, W) chmax(ans, a[i + 1][j + 1]);
  cout << ans << endl;
}