...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    rep(i, (1 << N)) {
      ...
      rep(j, N) if ((i >> j) & 1) temp1 += S1[j];
      rep3(j, N - 1, 0) if (not((i >> j) & 1)) temp3 += S1[j];
      rep(j, N) if ((i >> j) & 1) temp2 += S2[j];
      rep3(j, N - 1, 0) if (not((i >> j) & 1)) temp4 += S2[j];
      s1.push_back({temp1, temp3});
      s2[{temp2, temp4}]++;
    }
    rfor(i, s1) {
      if (s2.count({i[1], i[0]})) ans += s2[{i[1], i[0]}];
    }
    cout << ans << endl;
  }
};
