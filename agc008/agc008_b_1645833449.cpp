...
class BContiguousRepainting {
  public:
    static void solve(istream &cin, ostream &cout) {
        ...
        rfor(i, A) cin >> i;
        V<int> A_cum(N + 1);
        V<int> A_cum_plus(N + 1);
        rep(i, N) {
            A_cum[i + 1]      = A[i];
            A_cum_plus[i + 1] = A[i] > 0 ? A[i] : 0;
        }
        rep(i, N - 1) {
            A_cum[i + 2] += A_cum[i + 1];
            A_cum_plus[i + 2] += A_cum_plus[i + 1];
        }
        int ans = 0;
        rep(i, N - K + 1) {
            chmax(ans, A_cum_plus[i] + A_cum_plus[N] - A_cum_plus[i + K]);
            chmax(ans, A_cum_plus[i] + A_cum_plus[N] - A_cum_plus[i + K] + A_cum[i + K] - A_cum[i]);
        }
        cout << ans << endl;
    }
};
