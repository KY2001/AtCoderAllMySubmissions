...
struct Rolling_Hash {
  string S;
  vector<int> MODs = {1000000007, 998244353, 100000007}, B = {100007, 1007, 3571};
  vector<vector<int>> H;
...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    rep(i, S.size()) {
      ...
      if (rhS.Hash(i, i + t.size()) == rht.Hash(0, t.size())) {
        chmin(L, i);
        chmax(R, (int)(i + t.size()));
        cnt += t.size();
        i += t.size() - 1;
      }
    }
    if (R - L == cnt and cnt != 0) {
      if (cnt == S.size()) {
        cout << -1 << endl;
        return;
      } else if (S.substr(R, S.size() - R) + S.substr(0, L) == t) {
        cout << -1 << endl;
        return;
      }
    }
  ...
  return 0;
			...
			<div class="panel panel-default"><table class="table table-bordered table-striped">
				<tr>
					...
					<td class="text-center"><time class='fixtime fixtime-second'>2021-10-07 18:37:54+0900</time></td>
				</tr>
				<tr>
					<th>Task</th>
				...
				<tr>
					<th>Code Size</th>
					<td class="text-center">5545 Byte</td>
				</tr>
				<tr>
					<th>Status</th>
					...
					<tr>
						<th>Memory</th>
						<td class="text-center">202524 KB</td>
					</tr>
...
./Main.cpp:42:25: warning: comparison of integer expressions of different signedness: ‘long long int’ and ‘std::__cxx11::basic_string<char>::size_type’ {aka ‘long unsigned int’} [-Wsign-compare]
   42 |       for (int j = 1; j <= S.size(); j++) H[i][j] = (H[i][j - 1] * B[i] + S[j - 1]) % MODs[i];
      |                       ~~^~~~~~~~~~~
...
./Main.cpp:45:25: warning: comparison of integer expressions of different signedness: ‘long long int’ and ‘std::__cxx11::basic_string<char>::size_type’ {aka ‘long unsigned int’} [-Wsign-compare]
   45 |       for (int j = 0; j < S.size(); j++) pow[i][j + 1] = (pow[i][j] * B[i]) % MODs[i];
      |                       ~~^~~~~~~~~~
...
./Main.cpp:49:23: warning: comparison of integer expressions of different signedness: ‘long long int’ and ‘std::vector<long long int>::size_type’ {aka ‘long unsigned int’} [-Wsign-compare]
   49 |     for (int i = 0; i < MODs.size(); i++) ret.emplace_back((H[i][R] - H[i][L] * pow[i][R - L]) % MODs[i]);
      |                     ~~^~~~~~~~~~~~~
./Main.cpp:50:23: warning: comparison of integer expressions of different signedness: ‘long long int’ and ‘std::vector<long long int>::size_type’ {aka ‘long unsigned int’} [-Wsign-compare]
   50 |  ...