...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    shortest[1][1] = 0;
    priority_queue<V<int>, V<V<int>>, greater<V<int>>> pos;
    pos.push({0, 1, 1});
    while (pos.size()) {
      int y = pos.top()[1], x = pos.top()[2];
      pos.pop();
      if (shortest[y][x + 1] > shortest[y][x] + A[y][x] and x < C and shortest[y][x] + A[y][x] < shortest[R][C]) {
        shortest[y][x + 1] = shortest[y][x] + A[y][x];
        pos.push({shortest[y][x + 1], y, x + 1});
      }
      if (shortest[y][x - 1] > shortest[y][x] + A[y][x - 1] and x > 1) {
        shortest[y][x - 1] = shortest[y][x] + A[y][x - 1];
        pos.push({shortest[y][x - 1], y, x - 1});
      }
      if (shortest[y + 1][x] > shortest[y][x] + B[y][x] and y < R) {
        shortest[y + 1][x] = shortest[y][x] + B[y][x];
        pos.push({shortest[y + 1][x], y + 1, x});
      }
      rep2(i, 1, y) {
        if (shortest[y - i][x] > shortest[y][x] + 1 + i) {
          shortest[y - i][x] = shortest[y][x] + 1 + i;
          pos.push({shortest[y - i][x], y - i, x});
        }
      }
    }
