  ...
  static void solve(istream &cin, ostream &cout) {
    ...
    rep(i, T.size()-1) {
      if (T[i] == T[i + 1]) {
        ...
        check[i + 1] = false;
        rep(j, T.size()) {
          if (i - j - 1 < 0 or i + j + 1 > T.size() - 1) break;
          if (T[i - j - 1] == T[i + j + 1]) {
            check[i - j - 1] = false;
            check[i + j + 1] = false;
          }else{
  ...
  return 0;
			...
			<div class="panel panel-default"><table class="table table-bordered table-striped">
				<tr>
					...
					<td class="text-center"><time class='fixtime fixtime-second'>2021-05-01 21:30:21+0900</time></td>
				</tr>
				<tr>
					<th>Task</th>
				...
				<tr>
					<th>Code Size</th>
					<td class="text-center">3970 Byte</td>
				</tr>
				<tr>
					<th>Status</th>
