...
bool time_end() {
    if ((clock() - start) / CLOCKS_PER_SEC > 1.95) return true;
    return false;
}
...
void task(V<int>& choice) {
    ...
    if (T < best_T) {
        rep(i, 50) best_choice[i] = choice[i];
        rep(i, 102) best_xy[i] = xy[i];
        best_T = T;
    }
}
...
signed main() {
    ...
    V<int> choice_all(1000);
    V<int> choice(50);
    rep(i, 1000) choice_all[i] = i;
    while (!time_end()) {
        shuffle(all(choice_all), engine);
        rep(i, 50) choice[i] = choice_all[i];
        task(choice);
    }
    cout << m << " ";
