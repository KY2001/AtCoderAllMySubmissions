...
void choose_init_choice() {
    V<P<int, int>> dis(1000);
    rep(i, 1000) dis[i] = {calc_dis(1000, i), i};
    sort(all(dis));
    rep(i, 50) best_choice[i] = dis[i].se;
    task(best_choice);
}
...
signed main() {
    ...
    double num = 5;
    while (!time_end()) {
        change_num = 1 + (int)num * exp(-(clock() - start) / CLOCKS_PER_SEC * 3);
        cnt++;
        V<bool> contain(1000);
    ...
    //cout << round((double)_pow(10, 8) / (double)(1000 + best_T)) << endl;
}
