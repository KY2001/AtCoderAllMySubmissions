...
public:
  ...
  reduction(int a, int b) : first(a), second(b) { // a/b
    ...
    int num = (a < 0) + (b < 0);
    if (num % 2 == 0) first = abs(first), second = abs(second);
    if (num % 2 == 1) first = -abs(first), second = abs(second);
  }
};
...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    rep(i, N) {
      ...
      reduction<int> rd2(B, A);
      I[{rd.first, rd.second}]++;
      J[{-rd2.first, rd2.second}]++;
      IJ[i] = {{rd.first, rd.second}, {-rd2.first, rd2.second}};
    }
    int ans = 1;
    int num = N - SUM(zero);
    ...
    rep(i, N) {
      int a = J[IJ[i][0]];
      int b = I[IJ[i][0]];
      if (IJ[i][0].first == 0 or IJ[i][0].second == 0) continue;
      if (IJ[i][0] == IJ[i][1]) {
        if (a) {
          ans *= (a + 1);
          ans %= MOD;
