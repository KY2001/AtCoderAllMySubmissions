...
class C {
  public:
    static void solve(istream &cin, ostream &cout) {
        ...
        int A, B;
        cin >> A >> B;
        VV<bool> coprime(B - A + 1, V<bool>(B - A + 1));
        rep2(i, A, B + 1) rep2(j, A, B + 1) if (gcd(i, j) == 1) coprime[i - A][j - A] = true;
        int ans                                                                       = 0;
        auto dfs                                                                      = [&](auto self, V<int> &already, int R) -> void {
            if (R == B + 1) return;
            bool possible                                            = true;
            rfor(i, already) if (not coprime[i - A][R - A]) possible = false;
            if (possible) {
                already.emplace_back(R);
                self(self, already, R + 1);
                ans++;
                already.pop_back();
            }
            self(self, already, R + 1);
