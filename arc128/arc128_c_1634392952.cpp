...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    V<double> mean(N);
    mean[N - 1]               = A[N - 1];
    rep3(i, N - 2, 0) mean[i] = (mean[i + 1] * (N - i - 1) + A[i]) / (N - i);
    int R                     = N;
    double ans                = 0;
    V<double> check(N);
    while (1) {
      double max = 0;
      rep(i, R) chmax(max, mean[i]);
      rep3(i, R - 1, 0) if (mean[i] == max) {
        double num = R - i;
        if (M * num >= S) {
          ans += mean[i] * S;
          rep2(j, i, R) check[j] = S / num;
          S                          = 0;
        } else {
          ans += mean[i] * M * num;
          S -= M * num;
          rep2(j, i, R) check[j] = M;
          R                          = i;
        }
        break;
      }
      if (S == 0) break;
      mean[R - 1]               = A[R - 1];
      rep3(i, R - 2, 0) mean[i] = (mean[i + 1] * (R - i - 1) + A[i]) / (R - i);
    }
    cout << ans << endl;
  }
};
