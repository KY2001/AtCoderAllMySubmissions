...
signed main() {
  ...
  int num = 1;
  int neo = clock();
  map<int, vector<int>> decided;
  while (((clock()) - start) / CLOCKS_PER_SEC < 2.97) {
    if ((clock() - neo) / CLOCKS_PER_SEC > 0.05 and num < 10) {
      num++;
      neo = clock();
    }
    ...
    rep(i, n - 1) {
      if (when[i]) {
        if (decided.count(i)) {
          vector<int> temp = decided[i];
          int which        = temp[0];
          take_temp(which);
          pair<int, int> where = {temp[1], temp[2]};
          ...
          already[x[which]][y[which]] = true;
          memo.emplace_back();
          memo[memo.size() - 1] = {i, which, where.first, where.second};
        }
      }
      if (in_x(i) and in_y(i)) {
        take_temp(i + 1);
