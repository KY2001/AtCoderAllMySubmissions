...
signed main() {
  ...
  rep(i, n - 1) {
    ...
    int score_max = 0;
    rep2(j, i + 2, n - 2) {
      if (in_x(i, j) and in_y(i, j) and abs(x[j] - X) + abs(y[j] - Y) != 2) {
  ...
  int num    = 1;
  double neo = clock();
  while (((clock()) - start) / CLOCKS_PER_SEC < 2.9) {
    if ((clock() - neo) / CLOCKS_PER_SEC > 0.9 and num < 10) {
      num++;
      neo = clock();
    }
    X = 0;
    ...
    rep(i, n - 1) {
      if (when[i]) {
        std::uniform_int_distribution<int> dist_a(i + 1, n);
        std::uniform_int_distribution<int> dist_xy(0, 19);
        int which = dist_a(mt);
        take_temp(which);
        pair<int, int> where = {dist_xy(mt), dist_xy(mt)};
        while (!already[where.first][where.second]) where = {dist_xy(mt), dist_xy(mt)};
        already[x[which]][y[which]] = false;
        put_temp(where.first, where.second);
        x[which] = where.first, y[which] = where.second;
        already[where.first][where.second] = true;
      }
      int score_max = 0;
      rep2(j, i + 2, n - 2) {
