...
bool in_x(int i) {
  return (X <= x[i + 1] and x[i + 1] <= x[i]) or (x[i] <= x[i + 1] and x[i + 1] <= X or abs(X - x[i + 1]) <= 3);
}

bool in_y(int i) {
  return (Y <= y[i + 1] and y[i + 1] <= y[i]) or (y[i] <= y[i + 1] and y[i + 1] <= Y or abs(Y - y[i + 1]) <= 3);
}
  ...
  cout << ans << endl;
			...
			<div class="panel panel-default"><table class="table table-bordered table-striped">
				<tr>
					...
					<td class="text-center"><time class='fixtime fixtime-second'>2020-11-07 19:27:05+0900</time></td>
				</tr>
				<tr>
					<th>Task</th>
				...
				<tr>
					<th>Score</th>
					<td class="text-center">140636</td>
				</tr>
				<tr>
					<th>Code Size</th>
					<td class="text-center">5770 Byte</td>
				</tr>
				<tr>
					<th>Status</th>
					...
					<tr>
						<th>Exec Time</th>
						<td class="text-center">3000 ms</td>
					</tr>
					<tr>
						<th>Memory</th>
				...
				<h4>Compile Error</h4>
				<pre>./Main.cpp: In function ‘bool in_x(int)’:
./Main.cpp:94:68: warning: suggest parentheses around ‘&&’ within ‘||’ [-Wparentheses]
   94 |   return (X <= x[i + 1] and x[i + 1] <= x[i]) or (x[i] <= x[i + 1] and x[i + 1] <= X or abs(X - x[i + 1]) <= 3);
./Main.cpp: In function ‘bool in_y(int)’:
./Main.cpp:98:68: warning: suggest parentheses around ‘&&’ within ‘||’ [-Wparentheses]
   98 |   return (Y <= y[i + 1] and y[i + 1] <= y[i]) or (y[i] <= y[i + 1] and y[i + 1] <= Y or abs(Y - y[i + 1]) <= 3);
