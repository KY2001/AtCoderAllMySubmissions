...
template<class T>
inline int Lower_bound(vector<T> A, T key) { //key以上の最小のindex, 存在しない場合-1を返す。
  int ind = LOWER_BOUND(A, key);
  if (0 <= ind and ind < A.size()) return ind;
  return -1;
}
template<class T>
inline int Lower_bound_reverse(vector<T> A, T key) { //key以下の最大のindex, 存在しない場合-1を返す。
  int ind = UPPER_BOUND(A, key) - 1;
  if (0 <= ind and ind < A.size()) return ind;
  return -1;
}
template<class T>
int Upper_bound(vector<T> A, T key) { //keyより大きい最小のindex, 存在しない場合-1を返す。
  int ind = UPPER_BOUND(A, key);
  if (0 <= ind and ind < A.size()) return ind;
  return -1;
}
template<class T>
int Upper_bound_reverse(vector<T> A, T key) { //keyより小さい最大のindex, 存在しない場合-1を返す。
  ...
  cout << ans << endl;
			...
			<div class="panel panel-default"><table class="table table-bordered table-striped">
				<tr>
					...
					<td class="text-center"><time class='fixtime fixtime-second'>2020-11-09 11:27:39+0900</time></td>
				</tr>
				<tr>
					<th>Task</th>
				...
				<tr>
					<th>Code Size</th>
					<td class="text-center">4607 Byte</td>
				</tr>
				<tr>
					<th>Status</th>
					...
					<tr>
						<th>Exec Time</th>
						<td class="text-center">994 ms</td>
					</tr>
					<tr>
						<th>Memory</th>
						<td class="text-center">20180 KB</td>
					</tr>
...
./Main.cpp:31:24: warning: comparison of integer expressions of different signedness: ‘int’ and ‘std::vector<int>::size_type’ {aka ‘long unsigned int’} [-Wsign-compare]
   31 |   if (0 <= ind and ind < A.size()) return ind;
./Main.cpp: In instantiation of ‘int Lower_bound_reverse(std::vector<_Tp>, T) [with T = int]’:
./Main.cpp:95:51:   required from here
./Main.cpp:37:24: warning: comparison of integer expressions of different signedness: ‘int’ and ‘std::vector<int>::size_type’ {aka ‘long unsigned int’} [-Wsign-compare]
   37 |   if (0 <= ind and ind < A.size()) return ind;
