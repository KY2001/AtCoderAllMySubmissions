...
class DAtCoderExpress {
  public:
    static void solve(istream &cin, ostream &cout) {
        ...
        int N;
        cin >> N;
        V<double> t(N), v(N), t_sum(N), tv(N);
        rfor(i, t) cin >> i;
        rfor(i, v) cin >> i;
        t_sum                      = t;
        rep(i, N - 1) t_sum[i + 1] = t_sum[i] + t[i + 1];
        rep(i, N) tv[i]            = t_sum[i - 1] + v[i];
        tv.emplace_back(t_sum[N - 1]);
        rep3(i, N - 1, 0) tv[i] = min(tv[i], tv[i + 1]);
        double t_now = 0, v_now = 0, ans = 0;
        double gap = 1e-4;
        rep(i, N) {
            for (double j = 0; j < t[i]; j += gap) {
                double v_prev = v_now;
                if (v_now > tv[i + 1] - t_now - gap) {
                    v_now -= gap;
                } else if (v_now + gap > tv[i + 1] - t_now - gap or v_now >= v[i]) {
                } else {
                    v_now += gap;
                }
                ans += (v_now + v_prev) * gap / 2;
                t_now += gap;
            }
        }
        cout << ans << endl;
