  ...
  public:
    static void solve(istream &cin, ostream &cout) {
        ...
        rep(i, N) {
            LR[i] = {0, N - 1};
            if (index.size()) {
                auto L = index.lower_bound(a[i].se);
                auto R = index.upper_bound(a[i].se);
                if (L != begin(index)) LR[i].fi = *prev(L) + 1;
                if (R != end(index)) LR[i].se = *R - 1;
            }
            index.insert(a[i].se);
        }
        int ans = 0;
        rep(i, N) {
            ans += (LR[i].se - a[i].se + 1) * (a[i].se - LR[i].fi + 1) * a[i].fi;
        }
        cout << ans << endl;
    }
};
