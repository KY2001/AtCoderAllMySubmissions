...
public:
  static void solve(istream &cin, ostream &cout) {
    ...
    rep(i, N - 1) rep(j, N - 1) if (!check[i + 1][j + 1]) {
      ans++;
      deque<P<int, int>> pos = {{i, j}};
      while (pos.size()) {
        auto [y, x] = pos[0];
        pos.pop_front();
        if (check[y + 1][x + 1]) continue;
        check[y + 1][x + 1]++;
        pos.emplace_back(y + 1, x);
        pos.emplace_back(y - 1, x);
        pos.emplace_back(y, x + 1);
        pos.emplace_back(y, x - 1);
      }
    }
    cout << ans << endl;
  ...
  return 0;
			...
			<div class="panel panel-default"><table class="table table-bordered table-striped">
				<tr>
					...
					<td class="text-center"><time class='fixtime fixtime-second'>2021-05-03 02:39:23+0900</time></td>
				</tr>
				<tr>
					<th>Task</th>
				...
				<tr>
					<th>Code Size</th>
					<td class="text-center">4852 Byte</td>
				</tr>
				<tr>
					<th>Status</th>
					...
					<tr>
						<th>Exec Time</th>
						<td class="text-center">124 ms</td>
					</tr>
					<tr>
						<th>Memory</th>
						<td class="text-center">19468 KB</td>
					</tr>
...
./Main.cpp:81:7:   required from here
